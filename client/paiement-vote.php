...
$page_title = "Paiement de votre Vote - Tikéli";
...
<div class="payment-container" style="max-width: 580px; margin: 2rem auto; padding: 0 clamp(0.75rem, 2vw, 1rem);">
    <a href="accueil.php?onglet=voter" class="back-link"
    ...
    <div class="payment-card eventia-card"
        ...
        <div class="payment-heading"
            ...
            <span class="page-kicker"
                style="color: var(--tikeli-orange, #FF4A0D); font-weight: 700; font-size: 0.82rem; text-transform: uppercase; letter-spacing: 0.06em;">Vote
                #<?php echo (int) $vote_pay['id']; ?></span>
            <h1
                ...
                Finaliser votre Vote</h1>
            <p style="color: #737373; font-size: 0.92rem; margin: 0;">
                Votre vote pour <strong><?php echo htmlspecialchars($vote_pay['event_nom']); ?></strong> sera
                comptabilisé dès la confirmation du paiement.
            </p>
        </div>
        ...
        <?php if (!empty($candidats_choisis)): ?>
            ...
            <div
                style="background: #F5F5F5; border-bottom: 1px solid var(--eventia-border, #E5E5E5); padding: 1.25rem 2rem;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem;">
                    <span
                        style="color: var(--eventia-navy, #000000); font-family: var(--font-heading, 'Outfit', sans-serif); font-weight: 700; font-size: 0.88rem; text-transform: uppercase; letter-spacing: 0.5px;">
                        <i class="fa-solid fa-user-check" style="color: var(--eventia-amber-dark, #FF4A0D);"></i> Choix
                        sélectionné(s) (<?php echo count($candidats_choisis); ?>)
                    </span>
                ...
                <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                    <?php foreach ($candidats_choisis as $c): ?>
                        ...
                        ?>
                        <div
                            style="display: flex; gap: 1rem; align-items: center; background: #ffffff; border: 1px solid var(--eventia-border, #E5E5E5); border-radius: var(--eventia-radius-md, 10px); padding: 0.75rem 1rem; box-shadow: var(--eventia-shadow-xs);">
                            <img src="<?php echo $photo_url; ?>" alt="<?php echo htmlspecialchars($c['nom']); ?>"
                                style="width: 56px; height: 56px; border-radius: 50%; object-fit: cover; border: 2px solid var(--eventia-amber, #FF4A0D); flex-shrink: 0;">
                            <div style="flex: 1; min-width: 0;">
                                <strong
                                    style="color: var(--eventia-navy, #000000); font-family: var(--font-heading, 'Outfit', sans-serif); font-size: 0.95rem; display: block;"><?php echo htmlspecialchars($c['nom']); ?></strong>
                                <?php if (!empty($c['description'])): ?>
                                    <p
                                        style="color: var(--eventia-muted, #737373); font-size: 0.82rem; margin: 0.2rem 0 0; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">
                                        <?php echo htmlspecialchars($c['description']); ?>
                                    </p>
                                <?php endif; ?>
                            </div>
                            <span style="color: var(--eventia-turquoise, #FF4A0D); font-size: 1.1rem;"><i
        ...
        <div class="payment-amount"
            ...
            <div>
                <span
                    style="color: var(--eventia-navy, #000000); font-weight: 700; font-size: 0.95rem; display: block;">Montant
                    total du vote :</span>
                <?php if (!empty($candidats_choisis)): ?>
                    <small
            ...
            <strong class="swiss-numeral"
                style="color: var(--eventia-navy, #000000); font-size: 1.8rem; font-family: var(--font-heading, 'Outfit', sans-serif); font-weight: 800;"><?php echo number_format((float) $vote_pay['montant'], 0, ',', ' '); ?>
                <span
        ...
        <div class="payment-methods" style="padding: 2rem;">
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    const callbackUrl = baseUrl + '/callback-vote.php?vote_paiement_id=<?php echo $vote_pay['id']; ?>&methode=feexpay&vote_token=<?php echo $vote_token; ?>';
                    ...
                    FeexPayButton.init("render", {
                        ...
                        description: "Vote #<?php echo (int) $vote_pay['id']; ?>",
                        networks: {
