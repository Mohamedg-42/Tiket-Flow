...
<style>
    ...
    .search-input-field input, .search-input-field select {
        width: 100%;
        border: none;
        outline: none;
        padding: 0.5rem 0;
    ...
    .category-chip i { color: var(--primary); font-size: 0.9rem; }
    .category-chip:hover {
        border-color: var(--primary);
        color: var(--primary);
    ...
    .category-chip.active i { color: #ffffff; }
    ...
    .seat-map-toggle i { color: var(--primary); }
    .seat-map-toggle strong { color: var(--primary); }
    ...
    .seat-choice-label strong { color: var(--primary); }
    .seat-choice-label small { color: var(--muted); font-weight: 500; }
    .seat-map {
        display: grid;
    ...
    .seat-hint strong { color: var(--primary); }
    ...
    .vote-rank.top1 { background: #fef3c7; color: #b45309; }
    .vote-rank.top2 { background: #e2e8f0; color: #475569; }
    .vote-rank.top3 { background: #ffedd5; color: #ea580c; }
    .vote-btn {
        display: inline-flex;
        align-items: center;
        gap: 0.4rem;
    ...
    .vote-btn.voted i { color: #ffffff; }
</style>

<main class="client-main" style="max-width: 1200px; margin: 0 auto; padding: 2rem 1rem;">
    <!-- ===== Onglets principaux (tout en haut) ===== -->
    <div class="main-tabs">
        <a href="accueil.php?onglet=evenements" class="main-tab <?php echo ($onglet === 'evenements') ? 'active' : ''; ?>">
            <i class="fa-solid fa-calendar-days"></i> Événements
        </a>
        <a href="accueil.php?onglet=cotisations" class="main-tab <?php echo ($onglet === 'cotisations') ? 'active' : ''; ?>">
    ...
    <section>
        <div style="text-align: center; margin-bottom: 2rem;">
            ...
            <p style="color: var(--muted); font-size: 0.92rem; max-width: 560px; margin: 0.5rem auto 0;">
                Contribuez au financement des événements à venir. Chaque cotisation aide les promoteurs à organiser de plus grandes expériences.
            </p>
        </div>

        <?php if (!empty($cotisation_message)): ?>
            <div class="alert <?php echo ($cotisation_type === 'success') ? 'alert-success' : 'alert-error'; ?>" style="max-width: 640px; margin: 0 auto 1.5rem;">
            ...
            <?php foreach ($campagnes as $campagne): ?>
                ...
                <article class="event-card-item"<?php echo ($peut_contribuer && $peut_agir) ? ' style="cursor: pointer;" onclick="openCotisationModal(this.querySelector(\'button[data-campagne-id]\'))"' : ''; ?>>
                    <div style="position: relative;">
                        <img src="<?php echo $campagne_img; ?>" alt="<?php echo htmlspecialchars($campagne['titre']); ?>" class="event-poster">
                    ...
                    <div style="padding: 1.4rem; display: flex; flex-direction: column; flex: 1;">
                        ...
                        <div style="margin: 0 0 0.9rem;">
                            <?php if (!empty($camp_desc)): ?>
                                ...
                                <?php if ($camp_long): ?>
                                    <button type="button" onclick="event.stopPropagation(); toggleDesc('campagne_<?php echo (int)$campagne['id']; ?>', this)"
                        ...
                        <div style="font-size: 0.82rem; color: var(--ink); margin-bottom: 0.9rem; display: flex; align-items: center; gap: 0.5rem; font-weight: 600;">
                        ...
                        <div style="margin-top: auto; border-top: 1px solid var(--line-light); padding-top: 1rem;">
                            <div style="display: flex; justify-content: space-between; align-items: baseline; font-size: 0.78rem; font-weight: 700; margin-bottom: 6px;">
                            ...
                            <small style="color: var(--muted); display: block; margin-top: 6px; font-weight: 600;">
                                ...
                                <?php if ($objectif_atteint): ?>
                                    <span style="color: #16a34a;"><i class="fa-solid fa-circle-check"></i> Objectif atteint, les contributions restent ouvertes !</span>
                                <?php endif; ?>
                            </small>
                        </div>
                        ...
                        <div style="margin-top: 1.1rem;">
                            <?php if ($peut_contribuer && $peut_agir): ?>
                                <button type="button" class="btn-submit" style="width: 100%; margin: 0; padding: 0.7rem 1.25rem; font-size: 0.9rem;"
                            ...
                            <?php elseif (!$peut_agir): ?>
                                <span style="background: #fff7ed; color: #c2410c; border: 1px solid #fdba74; padding: 0.55rem 1rem; border-radius: 6px; font-weight: 800; font-size: 0.82rem; text-transform: uppercase; display: block; text-align: center;">
                                    <i class="fa-solid fa-lock"></i> Réservé aux clients
                                </span>
                            <?php else: ?>
                                <span style="background: #f1f5f9; color: #64748b; border: 1px solid #cbd5e1; padding: 0.55rem 1rem; border-radius: 6px; font-weight: 800; font-size: 0.82rem; text-transform: uppercase; display: block; text-align: center;">
                                    <i class="fa-solid fa-flag-checkered"></i> <?php echo $est_terminee ? 'Campagne terminée' : 'Objectif atteint'; ?>
                                </span>
                            <?php endif; ?>
                        </div>
                    </div>
        ...
        <div class="cotisation-card">
            ...
            <form method="POST" action="cotisation.php">
                ...
                <?php if ($is_logged_in): ?>
                    ...
                    <input type="hidden" name="nom" value="<?php echo htmlspecialchars($_SESSION['user_nom'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                    <input type="hidden" name="email" value="<?php echo htmlspecialchars($_SESSION['user_email'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                    <input type="hidden" name="telephone" value="<?php echo htmlspecialchars($user_telephone, ENT_QUOTES, 'UTF-8'); ?>">

                    <div style="background: #f0fdfa; border: 1px solid var(--line); border-left: 3px solid var(--primary); border-radius: 8px; padding: 0.7rem 0.9rem; margin-bottom: 1rem; font-size: 0.85rem; color: var(--ink);">
                        ...
                        <span style="color: var(--muted);">· <?php echo htmlspecialchars($user_telephone !== '' ? $user_telephone : ($_SESSION['user_email'] ?? '')); ?></span>
                    </div>
                <?php else: ?>
                    <!-- Visiteur : formulaire complet à remplir -->
                    <div class="form-group">
                        <label for="cot_nom"><i class="fa-solid fa-user"></i> Nom & Prénom *</label>
                        <input type="text" id="cot_nom" name="nom" required placeholder="Ex: Jean Koffi" value="<?php echo htmlspecialchars($_SESSION['user_nom'] ?? ''); ?>">
                    </div>

                    <div class="form-group">
                        <label for="cot_email"><i class="fa-regular fa-envelope"></i> Email</label>
                        <input type="email" id="cot_email" name="email" placeholder="votre.email@exemple.com" value="<?php echo htmlspecialchars($_SESSION['user_email'] ?? ''); ?>">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="cot_montant" name="montant" required min="500" step="500" placeholder="Ex: 5000">
                ...
                <p style="color: var(--muted); font-size: 0.78rem; margin-top: 0.75rem; text-align: center;">
                    <i class="fa-solid fa-shield-halved" style="color: var(--primary);"></i> Paiement sécurisé par Mobile Money (Wave, Orange, MTN, Moov)
                </p>
            </form>
            <?php else: ?>
            <div class="alert alert-error" style="margin: 0;">
                <i class="fa-solid fa-lock"></i>
                Les contributions sont réservées aux clients. Votre compte <?php echo htmlspecialchars($_SESSION['user_role'] ?? ''); ?> ne peut pas cotiser.
            </div>
            <?php endif; ?>
        </div>
        <?php endif; ?>
    ...
    <section>
        <div style="text-align: center; margin-bottom: 2rem;">
            ...
            <p style="color: var(--muted); font-size: 0.92rem; max-width: 560px; margin: 0.5rem auto 0;">
                Soutenez la réalisation des événements proposés par les promoteurs : les événements les plus votés sont mis en avant. Un vote par personne et par événement.
            </p>
        </div>
        ...
        try {
            $stmt_rank = $pdo->query("
                SELECT e.id, e.nom, e.categorie, e.date_evenement, e.lieu, e.image, e.description,
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 1.75rem;">
                <?php foreach ($classement as $rang => $ev): ?>
                    ...
                    <article class="event-card-item">
                        <div style="position: relative;">
                            <img src="<?php echo $ev_img; ?>" alt="<?php echo htmlspecialchars($ev['nom']); ?>" class="event-poster">
                            <span class="event-category-badge">
                        ...
                        <div style="padding: 1.4rem; display: flex; flex-direction: column; flex: 1;">
                            ...
                            <div style="font-size: 0.85rem; color: var(--muted); margin-bottom: 0.9rem; display: flex; flex-direction: column; gap: 0.3rem;">
                                <span><i class="fa-solid fa-tag" style="color: var(--primary);"></i> <?php echo htmlspecialchars($ev['categorie']); ?></span>
                                <span><i class="fa-solid fa-location-dot" style="color: #ef4444;"></i> <?php echo htmlspecialchars($ev['lieu']); ?></span>
                            </div>
                            ...
                            <div style="margin: 0 0 0.9rem;">
                                <?php if (!empty($vote_full)): ?>
                                    ...
                                    <?php if ($vote_long): ?>
                                        <button type="button" onclick="event.stopPropagation(); toggleDesc('vote_<?php echo (int)$ev['id']; ?>', this)"
                                                style="background: none; border: none; padding: 0; margin-top: 0.3rem; color: var(--primary); font-size: 0.8rem; font-weight: 700; cursor: pointer; font-family: inherit; display: inline-flex; align-items: center; gap: 0.3rem;">
                                            Voir plus <i class="fa-solid fa-chevron-down"></i>
                                        </button>
                                    <?php endif; ?>
                            ...
                            <div style="margin-top: auto; border-top: 1px solid var(--line-light); padding-top: 1rem; display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <button type="button" class="vote-btn <?php echo $deja_vote ? 'voted' : ''; ?>"
                                        ...
                                        <?php if ($peut_agir): ?>onclick="toggleVote(event, this)"<?php else: ?>disabled title="Réservé aux clients"<?php endif; ?>>
                                    <i class="fa-solid <?php echo $peut_agir ? 'fa-thumbs-up' : 'fa-lock'; ?>"></i>
                                    <?php echo $peut_agir ? ($deja_vote ? 'Voté' : 'Voter') : 'Réservé aux clients'; ?>
                                </button>
                            </div>
                        </div>
                    </article>
        ...
        <?php else: ?>
            <div style="text-align: center; background: #ffffff; border: 1px solid var(--line); border-radius: var(--radius-xl); padding: 3.5rem 1rem; max-width: 640px; margin: 0 auto;">
                <i class="fa-solid fa-vote-yea" style="font-size: 3rem; color: var(--line); margin-bottom: 1rem; display: block;"></i>
                ...
                <a href="accueil.php?onglet=evenements" class="btn-submit" style="display: inline-block; width: auto; text-decoration: none; padding: 0.65rem 1.5rem;">
                    Voir les événements
                </a>
            </div>
        <?php endif; ?>
    ...
    <!-- ============================================================
         ...
         ============================================================ -->
    <!-- Bulles de filtre par catégorie -->
    <div class="category-chips">
        <a href="accueil.php" class="category-chip <?php echo empty($categorie) ? 'active' : ''; ?>"><i class="fa-solid fa-border-all"></i> Tous</a>
        <a href="accueil.php?categorie=Concert" class="category-chip <?php echo ($categorie === 'Concert') ? 'active' : ''; ?>"><i class="fa-solid fa-music"></i> Concert</a>
        <a href="accueil.php?categorie=Festival" class="category-chip <?php echo ($categorie === 'Festival') ? 'active' : ''; ?>"><i class="fa-solid fa-umbrella-beach"></i> Festival</a>
        <a href="accueil.php?categorie=Spectacle" class="category-chip <?php echo ($categorie === 'Spectacle') ? 'active' : ''; ?>"><i class="fa-solid fa-masks-theater"></i> Spectacle</a>
        <a href="accueil.php?categorie=Conférence" class="category-chip <?php echo ($categorie === 'Conférence') ? 'active' : ''; ?>"><i class="fa-solid fa-microphone"></i> Conférence</a>
        <a href="accueil.php?categorie=Sport" class="category-chip <?php echo ($categorie === 'Sport') ? 'active' : ''; ?>"><i class="fa-solid fa-futbol"></i> Sport</a>
        <a href="accueil.php?categorie=Soirée" class="category-chip <?php echo ($categorie === 'Soirée') ? 'active' : ''; ?>"><i class="fa-solid fa-champagne-glasses"></i> Soirée</a>
    </div>
    ...
    <section class="hero-banner">
        ...
        <form method="GET" action="accueil.php" class="search-box-wrapper">
            <div class="search-input-field">
                ...
                <input type="text" name="q" placeholder="Artiste, groupe, concert..." value="<?php echo htmlspecialchars($q); ?>">
            </div>

            <div class="search-input-field">
                <i class="fa-solid fa-location-dot"></i>
                <input type="text" name="lieu" placeholder="Ville ou salle (Abidjan, Dakar...)" value="<?php echo htmlspecialchars($lieu); ?>">
            </div>
            ...
            <button type="submit" class="btn-submit" style="width: auto; margin: 0; padding: 0.75rem 1.75rem; border-radius: 8px;">
                Rechercher
            </button>
        </form>
    </section>
    ...
    <section>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 1.75rem;">
                <?php foreach ($events as $event): ?>
                    ...
                    <article class="event-card-item"<?php echo ($event['statut'] !== 'termine' && $peut_agir) ? ' style="cursor: pointer;" onclick="openEventModal(this.querySelector(\'button[data-ticket-options]\'))"' : ''; ?>>
                        <div style="position: relative;">
                            <img src="<?php echo $img_url; ?>" alt="<?php echo htmlspecialchars($event['nom']); ?>" class="event-poster">
                            <span class="event-category-badge"><?php echo htmlspecialchars($event['categorie']); ?></span>
                            <span class="event-date-chip">
                                <i class="fa-regular fa-calendar" style="color: var(--primary);"></i> <?php echo date('d/m/Y', strtotime($event['date_evenement'])); ?>
                            </span>
                            <button type="button" class="event-like-btn <?php echo !empty($liked_events[$event['id']]) ? 'liked' : ''; ?>"
                                    data-event-id="<?php echo (int)$event['id']; ?>"
                                    <?php if ($peut_agir): ?>onclick="toggleLike(event, this)" aria-label="J'aime cet événement"<?php else: ?>disabled title="Réservé aux clients" style="opacity: 0.55; cursor: not-allowed;"<?php endif; ?>>
                        ...
                        <div style="padding: 1.4rem; display: flex; flex-direction: column; flex: 1;">
                            ...
                            <div style="margin: 0 0 1.25rem; flex: 1;">
                                <?php if (!empty($full_desc)): ?>
                                    ...
                                    <?php if ($desc_long): ?>
                                        <button type="button" onclick="event.stopPropagation(); toggleDesc(<?php echo (int)$event['id']; ?>, this)"
                                                style="background: none; border: none; padding: 0; margin-top: 0.3rem; color: var(--primary); font-size: 0.8rem; font-weight: 700; cursor: pointer; font-family: inherit; display: inline-flex; align-items: center; gap: 0.3rem;">
                                            Voir plus <i class="fa-solid fa-chevron-down"></i>
                                        </button>
                                    <?php endif; ?>
                            ...
                            <?php if (!empty($event_tickets)): ?>
                                <div style="display: grid; gap: 0.55rem; margin-bottom: 1.25rem;">
                                    <?php foreach ($event_tickets as $tk): ?>
                                        ...
                                        ?>
                                        <div>
                                            <div style="display: flex; justify-content: space-between; align-items: center; font-size: 0.76rem; font-weight: 700; margin-bottom: 3px;">
                                                <span style="color: var(--navy);">
                                                    <i class="fa-solid fa-ticket" style="color: var(--primary); font-size: 0.68rem;"></i>
                                            ...
                                            <div style="height: 6px; background: #e2e8f0; border-radius: 999px; overflow: hidden;">
                                                <div style="height: 100%; width: <?php echo $pct_vendu; ?>%; background: <?php echo $epuise ? '#ef4444' : 'linear-gradient(90deg, var(--primary), var(--primary-light))'; ?>; border-radius: 999px; transition: width 0.4s ease;"></div>
                                            </div>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            ...
                            <div style="font-size: 0.88rem; color: var(--ink); margin-bottom: 1.25rem; display: flex; align-items: center; gap: 0.5rem;">
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid var(--line-light); padding-top: 1rem;">
                                <div>
                                    <small style="color: var(--muted); display: block; font-size: 0.72rem; text-transform: uppercase; font-weight: 700;">À partir de</small>
                                    <strong style="color: var(--primary); font-size: 1.15rem;"><?php echo number_format($prix_min, 0, ',', ' '); ?> FCFA</strong>
                                </div>

                                <?php if ($event['statut'] === 'termine'): ?>
                                    <span style="background: #f1f5f9; color: #64748b; border: 1px solid #cbd5e1; padding: 0.55rem 1rem; border-radius: 6px; font-weight: 800; font-size: 0.82rem; text-transform: uppercase;">
                                        <i class="fa-solid fa-flag-checkered"></i> Terminé
                                    </span>
                                <?php else: ?>
                                    <button type="button" class="btn-submit" style="width: auto; margin: 0; padding: 0.65rem 1.25rem; font-size: 0.88rem;"
        ...
        <?php else: ?>
            <div style="text-align: center; background: #ffffff; border: 1px solid var(--line); border-radius: var(--radius-xl); padding: 4rem 1rem;">
                <i class="fa-regular fa-calendar-xmark" style="font-size: 3.5rem; color: var(--line); margin-bottom: 1rem; display: block;"></i>
                <h3 style="color: var(--navy); margin-bottom: 0.5rem;">Aucun événement ne correspond à votre recherche</h3>
                <p style="color: var(--muted); margin-bottom: 1.5rem;">Modifiez vos filtres ou réessayez avec d'autres mots-clés.</p>
                <a href="accueil.php" class="btn-submit" style="display: inline-block; width: auto; text-decoration: none; padding: 0.75rem 1.75rem;">
...
<!-- =========================================================================
     ...
     ========================================================================= -->
<div id="clientEventModal" class="client-modal" role="dialog" aria-modal="true" aria-labelledby="clientModalTitle" hidden>
    <div class="client-modal-box" style="max-width: 540px;">
        ...
        <p class="client-modal-event-name" id="clientModalEventName" style="font-weight: 700; color: var(--primary); font-size: 1.05rem; margin-bottom: 0.4rem;"></p>

        <!-- Description complète -->
        <p id="clientModalDesc" style="color: var(--muted); font-size: 0.88rem; line-height: 1.55; margin-bottom: 1rem; display: none;"></p>

        <div style="background: #f8fafc; border-radius: 8px; padding: 0.75rem 1rem; margin-bottom: 1.25rem; font-size: 0.85rem; color: var(--muted); display: grid; gap: 0.35rem;">
            <div><i class="fa-regular fa-calendar" style="color: var(--primary);"></i> Date : <strong id="clientModalDate" style="color: var(--navy);"></strong> à <strong id="clientModalTime" style="color: var(--navy);"></strong></div>
            <div><i class="fa-solid fa-location-dot" style="color: #ef4444;"></i> Salle / Lieu : <strong id="clientModalPlace" style="color: var(--navy);"></strong></div>
            <div><i class="fa-solid fa-chair" style="color: var(--primary);"></i> Places disponibles : <strong id="clientModalCapacity" style="color: var(--navy);"></strong></div>
        </div>
        ...
        <form id="clientOrderForm" method="POST" action="commander.php">
            ...
            <div id="ticket-tiers-container" style="max-height: 220px; overflow-y: auto; margin-bottom: 1.25rem; padding-right: 2px;">
            ...
            <?php if ($is_logged_in): ?>
                ...
                <div style="background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 8px; padding: 0.75rem 1rem; margin-bottom: 1.25rem; font-size: 0.85rem; display: flex; align-items: center; gap: 0.65rem;">
                    <div style="width: 32px; height: 32px; border-radius: 50%; background: #dcfce7; color: #16a34a; display: grid; place-items: center; flex-shrink: 0;">
                        <i class="fa-solid fa-user-check"></i>
                    </div>
                    <div>
                        <span style="color: #166534; font-weight: bold; display: block;">Compte connecté : <?php echo htmlspecialchars($_SESSION['user_nom'] ?? 'Client'); ?></span>
                        <small style="color: var(--muted);"><?php echo htmlspecialchars($_SESSION['user_email'] ?? ''); ?> · Vos billets seront directement liés à votre compte</small>
                    </div>
                </div>
            <?php else: ?>
                <!-- Formulaire demandé UNIQUEMENT pour les visiteurs non connectés -->
                <div style="background: #f8fafc; border: 1px solid var(--line); border-radius: 8px; padding: 1rem; margin-bottom: 1.25rem;">
                    <div style="font-weight: 700; color: var(--navy); font-size: 0.85rem; margin-bottom: 0.75rem;">
                        <i class="fa-solid fa-address-card" style="color: var(--primary);"></i> Coordonnées de réception des billets
                    </div>
                    ...
                    <div class="form-group" style="margin-bottom: 0.75rem;">
                        <label for="client_email" style="font-size: 0.8rem;">Adresse Email (réception des billets & QR codes) *</label>
                        <input type="email" id="client_email" name="client_email" required placeholder="votre.email@exemple.com">
                    </div>

                    <div class="form-group" style="margin-bottom: 0;">
                        <label for="client_telephone" style="font-size: 0.8rem;">Numéro de Téléphone (Mobile Money) *</label>
                        <input type="tel" id="client_telephone" name="client_telephone" required placeholder="Ex: 07 00 00 00 00">
                    </div>
                </div>
            <?php endif; ?>

            <!-- Total à payer et décompte des places -->
            <div style="background: var(--navy); color: #ffffff; padding: 1rem 1.25rem; border-radius: 8px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.25rem;">
                <div>
                    <span style="font-weight: 600; font-size: 0.82rem; color: #94a3b8; display: block; text-transform: uppercase;">Total Commande</span>
                    <small id="clientModalTicketsCount" style="color: #38bdf8; font-weight: bold;">0 place(s) sélectionnée(s)</small>
                </div>
                <strong id="clientModalTotal" style="color: #38bdf8; font-size: 1.4rem;">0 FCFA</strong>
            </div>

            <div style="display: flex; gap: 0.75rem;">
                <button type="button" class="btn-submit" onclick="closeEventModal()" style="flex: 1; background: transparent; color: var(--muted); border: 1px solid var(--line);">
                    Annuler
                </button>
...
<!-- ============================================================
     ...
     ============================================================ -->
<div id="cotisationModal" class="client-modal" role="dialog" aria-modal="true" aria-labelledby="cotisationModalTitle" hidden>
    <div class="client-modal-box" style="max-width: 520px;">
        ...
        <p id="cotisationModalCampagneName" style="font-weight: 700; color: var(--primary); font-size: 0.95rem; margin: 0 0 0.75rem;">
            Contribution générale
        </p>

        <!-- Motivation complète du créateur de la campagne -->
        <p id="cotisationModalMotivation" style="display: none; color: var(--muted); font-size: 0.88rem; line-height: 1.55; background: var(--primary-soft, #f0fdfa); border-left: 3px solid var(--primary); border-radius: 6px; padding: 0.75rem 0.9rem; margin: 0 0 1rem;">
        </p>
