...
switch ($period) {
    case '7d':
        $sql_period_cur_pay = "p.date_paiement >= NOW() - INTERVAL '7 days'";
        $sql_period_prev_pay = "p.date_paiement BETWEEN NOW() - INTERVAL '14 days' AND NOW() - INTERVAL '7 days'";
        ...
        $sql_period_cur_ord = "o.created_at >= NOW() - INTERVAL '7 days'";
        $sql_period_prev_ord = "o.created_at BETWEEN NOW() - INTERVAL '14 days' AND NOW() - INTERVAL '7 days'";
        $points_count = 7;
        $step_days = 1;
    ...
    case 'this_month':
        $sql_period_cur_pay = "EXTRACT(MONTH FROM p.date_paiement) = EXTRACT(MONTH FROM CURRENT_DATE) AND EXTRACT(YEAR FROM p.date_paiement) = EXTRACT(YEAR FROM CURRENT_DATE)";
        $sql_period_prev_pay = "EXTRACT(MONTH FROM p.date_paiement) = EXTRACT(MONTH FROM CURRENT_DATE - INTERVAL '1 month') AND EXTRACT(YEAR FROM p.date_paiement) = EXTRACT(YEAR FROM CURRENT_DATE - INTERVAL '1 month')";
        ...
        $sql_period_cur_ord = "EXTRACT(MONTH FROM o.created_at) = EXTRACT(MONTH FROM CURRENT_DATE) AND EXTRACT(YEAR FROM o.created_at) = EXTRACT(YEAR FROM CURRENT_DATE)";
        $sql_period_prev_ord = "EXTRACT(MONTH FROM o.created_at) = EXTRACT(MONTH FROM CURRENT_DATE - INTERVAL '1 month') AND EXTRACT(YEAR FROM o.created_at) = EXTRACT(YEAR FROM CURRENT_DATE - INTERVAL '1 month')";
        $points_count = 6;
        $step_days = 5;
    ...
    case 'this_year':
        $sql_period_cur_pay = "EXTRACT(YEAR FROM p.date_paiement) = EXTRACT(YEAR FROM CURRENT_DATE)";
        $sql_period_prev_pay = "EXTRACT(YEAR FROM p.date_paiement) = EXTRACT(YEAR FROM CURRENT_DATE) - 1";
        ...
        $sql_period_cur_ord = "EXTRACT(YEAR FROM o.created_at) = EXTRACT(YEAR FROM CURRENT_DATE)";
        $sql_period_prev_ord = "EXTRACT(YEAR FROM o.created_at) = EXTRACT(YEAR FROM CURRENT_DATE) - 1";
        $points_count = 7;
        $step_days = 50;
    ...
    default:
        $period = '30d';
        $sql_period_cur_pay = "p.date_paiement >= NOW() - INTERVAL '30 days'";
        $sql_period_prev_pay = "p.date_paiement BETWEEN NOW() - INTERVAL '60 days' AND NOW() - INTERVAL '30 days'";
        ...
        $sql_period_cur_ord = "o.created_at >= NOW() - INTERVAL '30 days'";
        $sql_period_prev_ord = "o.created_at BETWEEN NOW() - INTERVAL '60 days' AND NOW() - INTERVAL '30 days'";
        $points_count = 7;
        $step_days = 5;
...
$stmt_top = $pdo->query("
    ...
    GROUP BY e.id, e.nom, e.image, e.date_evenement, e.heure, e.categorie, u.nom
...
<div class="dash-container">
    ...
    <div class="dash-header-section" style="margin-bottom: 1.25rem;">
        <div class="dash-title-box">
            ...
            <p>Bienvenue, <strong><?php echo htmlspecialchars($admin_fullname); ?></strong> · Données temps réel consolidées de la plateforme · Période :
    ...
    <div
        ...
        <div style="display: flex; gap: 0.4rem; align-items: center; flex-wrap: wrap;">
            <a href="?period=7d<?php echo $selected_event_id ? '&event_id=' . $selected_event_id : ''; ?>"
                style="text-decoration: none; border-radius: 9px; padding: 0.45rem 0.9rem; font-size: 0.82rem; font-weight: 700; display: inline-flex; align-items: center; gap: 5px; <?php echo $period === '7d' ? 'background: #000000; color: #ffffff; box-shadow: 0 4px 12px rgba(11, 29, 58, 0.25);' : 'background: #F5F5F5; color: #737373; border: 1px solid #E5E5E5;'; ?>">
                <i class="fa-regular fa-clock" style="<?php echo $period === '7d' ? 'color: var(--eventia-amber, #FF4A0D);' : ''; ?>"></i> 7 jours
            </a>

            <a href="?period=30d<?php echo $selected_event_id ? '&event_id=' . $selected_event_id : ''; ?>"
                style="text-decoration: none; border-radius: 9px; padding: 0.45rem 0.9rem; font-size: 0.82rem; font-weight: 700; display: inline-flex; align-items: center; gap: 5px; <?php echo $period === '30d' ? 'background: #000000; color: #ffffff; box-shadow: 0 4px 12px rgba(11, 29, 58, 0.25);' : 'background: #F5F5F5; color: #737373; border: 1px solid #E5E5E5;'; ?>">
                <i class="fa-solid fa-calendar-days"
                    style="<?php echo $period === '30d' ? 'color: var(--eventia-amber, #FF4A0D);' : ''; ?>"></i> 30 jours
            </a>

            <a href="?period=this_month<?php echo $selected_event_id ? '&event_id=' . $selected_event_id : ''; ?>"
                style="text-decoration: none; border-radius: 9px; padding: 0.45rem 0.9rem; font-size: 0.82rem; font-weight: 700; display: inline-flex; align-items: center; gap: 5px; <?php echo $period === 'this_month' ? 'background: #000000; color: #ffffff; box-shadow: 0 4px 12px rgba(11, 29, 58, 0.25);' : 'background: #F5F5F5; color: #737373; border: 1px solid #E5E5E5;'; ?>">
                <i class="fa-regular fa-calendar"
                    style="<?php echo $period === 'this_month' ? 'color: var(--eventia-amber, #FF4A0D);' : ''; ?>"></i> Ce mois
            </a>

            <a href="?period=this_year<?php echo $selected_event_id ? '&event_id=' . $selected_event_id : ''; ?>"
                style="text-decoration: none; border-radius: 9px; padding: 0.45rem 0.9rem; font-size: 0.82rem; font-weight: 700; display: inline-flex; align-items: center; gap: 5px; <?php echo $period === 'this_year' ? 'background: #000000; color: #ffffff; box-shadow: 0 4px 12px rgba(11, 29, 58, 0.25);' : 'background: #F5F5F5; color: #737373; border: 1px solid #E5E5E5;'; ?>">
                <i class="fa-solid fa-chart-pie"
                    style="<?php echo $period === 'this_year' ? 'color: var(--eventia-amber, #FF4A0D);' : ''; ?>"></i> Cette année
            </a>

            <a href="?period=all<?php echo $selected_event_id ? '&event_id=' . $selected_event_id : ''; ?>"
                style="text-decoration: none; border-radius: 9px; padding: 0.45rem 0.9rem; font-size: 0.82rem; font-weight: 700; display: inline-flex; align-items: center; gap: 5px; <?php echo $period === 'all' ? 'background: #000000; color: #ffffff; box-shadow: 0 4px 12px rgba(11, 29, 58, 0.25);' : 'background: #F5F5F5; color: #737373; border: 1px solid #E5E5E5;'; ?>">
                <i class="fa-solid fa-infinity" style="<?php echo $period === 'all' ? 'color: var(--eventia-amber, #FF4A0D);' : ''; ?>"></i>
                Tout
            </a>
        </div>
        ...
        <form method="GET" action="dashboard.php" id="filterForm"
            ...
            <div
                style="display: flex; align-items: center; gap: 6px; padding: 0.4rem 0.75rem; border-radius: 8px; border: 1px solid var(--dash-border); background: #F5F5F5;">
                <i class="fa-solid fa-filter" style="color: var(--dash-primary); font-size: 0.8rem;"></i>
                <select name="event_id" onchange="document.getElementById('filterForm').submit();"
                    style="border: none; background: transparent; font-weight: 700; color: var(--dash-text); outline: none; cursor: pointer; max-width: 220px; font-size: 0.82rem;">
            ...
            <?php if ($selected_event_id): ?>
                <a href="?period=<?php echo $period; ?>"
                    style="color: #000000; font-size: 0.78rem; text-decoration: underline;">Tous les événements</a>
            <?php endif; ?>
        </form>
    </div>
    ...
    <div class="dash-kpi-grid-6">
        ...
        <a href="paiements.php" class="dash-kpi-card" title="Voir tous les paiements et transactions">
            ...
            <div class="dash-kpi-amount"><?php echo number_format($total_revenue, 0, ',', ' '); ?> <small
                    style="font-size: 0.72rem; font-weight: 700;">F</small></div>
            <div class="dash-kpi-sub">Total encaissé sur la période <i class="fa-solid fa-arrow-right"
        ...
        <a href="commandes.php" class="dash-kpi-card" title="Voir toutes les commandes clients">
            <div class="dash-kpi-top">
                ...
                <span class="dash-kpi-badge-pill <?php echo $ord_growth['class']; ?>">
                    <i class="fa-solid fa-arrow-trend-<?php echo $ord_growth['dir'] === 'down' ? 'down' : 'up'; ?>"></i>
                    <?php echo $ord_growth['text']; ?>
                </span>
            </div>
            <div class="dash-kpi-title">Panier Moyen</div>
            <div class="dash-kpi-amount"><?php echo number_format($panier_moyen, 0, ',', ' '); ?> <small
                    style="font-size: 0.72rem; font-weight: 700;">F</small></div>
            <div class="dash-kpi-sub"><?php echo number_format($total_orders, 0, ',', ' '); ?> commandes payées <i
                    class="fa-solid fa-arrow-right" style="font-size: 0.65rem; margin-left: 2px;"></i></div>
            <div class="dash-sparkline-box">
            ...
            <div class="dash-card-head">
                ...
                <div class="dash-chart-tabs">
                    <button type="button" class="dash-chart-tab active"
                        onclick="switchChartMode('revenue', this)">Recettes (F)</button>
                    <button type="button" class="dash-chart-tab"
                        onclick="switchChartMode('tickets', this)">Billets</button>
                </div>
            </div>
                                    ...
                                    <td data-label="Événement">
                                        <div class="dash-event-cell">
                                            ...
                                            <div class="dash-event-meta">
                                                ...
                                                <small><i class="fa-solid fa-user-tie"></i>
                                                    <?php echo htmlspecialchars($ev['promoteur_nom'] ?? 'Organisateur'); ?></small>
                                            </div>
                                        </div>
                                    </td>
                                    <td data-label="Date" style="white-space: nowrap; font-size: 0.8rem;">
                                        <?php echo date('d/m/Y', strtotime($ev['date_evenement'])); ?></td>
                                    <td data-label="Vendus"><strong><?php echo number_format($vds, 0, ',', ' '); ?></strong>
                                    </td>
                                    <td data-label="Recette" style="font-weight: 700; color: var(--dash-primary);">
                        ...
                        <a href="commandes.php" class="dash-stream-card" title="Voir les commandes correspondantes">
                            <div class="dash-stream-left">
                                ...
                                <div class="dash-stream-texts">
                                    ...
                                    <span class="dash-stream-desc">Acheteur :
                                        <?php echo htmlspecialchars($act['client_nom'] ?: 'Client Web'); ?></span>
                                </div>
                            </div>
    ...
    <div class="dash-card" style="margin-bottom: 1.75rem;">
        <div class="dash-card-head">
            <div>
                <h3 class="dash-card-title">
                    <i class="fa-solid fa-layer-group" style="color: #FF4A0D;"></i>
            ...
            <span
                style="background: #FFF2ED; color: #FF4A0D; padding: 4px 10px; border-radius: 8px; font-size: 0.78rem; font-weight: 800;">
                        ...
                        <?php foreach ($tt_by_event as $tbe): ?>
                            <tr>
                                ...
                                <td data-label="Billets Vendus">
                                    <strong
                                        style="font-size: 0.92rem;"><?php echo number_format($tbe['nb_vendus'], 0, ',', ' '); ?></strong>
                                </td>
                                <td data-label="Recette">
                                    <strong
                                        style="color: var(--dash-primary); font-size: 0.92rem;"><?php echo number_format((float) $tbe['ca_type'], 0, ',', ' '); ?>
                                        F</strong>
                                </td>
                            </tr>
                        <?php endforeach; ?>
    ...
    <div
        ...
        <div>
            <span><i class="fa-solid fa-shield-check" style="color: #FF4A0D;"></i> Données synchronisées avec la base
                MySQL</span>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        window.switchChartMode = function (mode, btn) {
            ...
            if (mode === 'tickets') {
                ...
                mainChartInstance.data.datasets[0].borderColor = '#FF4A0D';
            ...
            new Chart(ctxPayment, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: ['#1dc4e9', '#FF4A0D', '#f59e0b', '#10b981'],
                        borderWidth: 0,
                        hoverOffset: 6
                    }]
                },
