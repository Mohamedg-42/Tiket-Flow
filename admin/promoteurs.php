...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_promoter'])) {
    ...
    if (empty($nom) || empty($prenom) || empty($email) || empty($password)) {
        ...
        if ($st_chk->fetch()) {
            ...
            $st_prom = $pdo->prepare("
                INSERT INTO promoters (user_id, type_entite, nom_commercial, numero_registre, representant_legal, telephone_contact, email_contact, ville, statut, solde, created_at)
                ...
                ON DUPLICATE KEY UPDATE
                    ...
                    statut = 'approuve'
            ");
            $st_prom->execute([$new_user_id, $type_entite, $nom_commercial, $rccm_val, $rep_val, $telephone, $email, $ville]);
            ...
            logActivity('user.create', 'user', $new_user_id, "Création manuelle du promoteur « $nom_commercial » ($type_entite)");

            $message = "Le promoteur « " . htmlspecialchars($nom_commercial) . " » a été créé et activé avec succès !" . ($mail_ok ? " Ses identifiants ont été envoyés à $email." : "");
            $msg_type = "success";
        }
    }
}
                    ...
                    <div class="promoter-mobile-card">
                        ...
                        <div class="pmc-actions">
                            <?php if ($is_susp): ?>
                                <a href="promoteurs.php?id=<?php echo $p['id']; ?>&action=activate"
                                    onclick="return confirm('Réactiver ce promoteur ?');" class="dash-btn-action"
                                    style="background: #FFF2ED; color: #000000; font-weight: 800;">
                                    <i class="fa-solid fa-unlock"></i> Réactiver le Promoteur
                                </a>
                            <?php else: ?>
                                <a href="promoteurs.php?id=<?php echo $p['id']; ?>&action=suspend"
                                    onclick="return confirm('Voulez-vous suspendre temporairement ce promoteur ?');"
                                    class="dash-btn-action"
                                    style="background: #F5F5F5; color: #000000; font-weight: 800;">
                                    <i class="fa-solid fa-ban"></i> Suspendre le Compte
                                </a>
                            <?php endif; ?>
                        </div>
                    </div>
