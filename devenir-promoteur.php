<?php
// ==============================================================================
// CANDIDATURE & DOSSIER D'ÉLIGIBILITÉ PROMOTEUR OFFICIEL (devenir-promoteur.php)
// Évaluation d'éligibilité légale : Personne Physique vs Personne Morale
// Compte Organisateur indépendant — Aucun compte client préalable n'est requis.
// ==============================================================================
...
// Pré-remplissage éventuel depuis la session si l'utilisateur est déjà connecté
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Récupération Ville & Commune (Listes déroulantes synchronisées)
    $ville_raw = trim($_POST['ville'] ?? '');
    $ville_autre = trim($_POST['ville_autre'] ?? '');
    $ville_nom = ($ville_raw === 'Autre' && !empty($ville_autre)) ? $ville_autre : $ville_raw;

    $commune_raw = trim($_POST['commune'] ?? '');
    $commune_autre = trim($_POST['commune_autre'] ?? '');
    $commune_nom = (!empty($commune_raw) && (strpos($commune_raw, 'Autre') === 0 || $commune_raw === 'Autre') && !empty($commune_autre)) ? $commune_autre : $commune_raw;

    // Concaténation unifiée pour affichage standard et compatibilité totale
    if (!empty($commune_nom) && $commune_nom !== $ville_nom) {
        $ville = $ville_nom . ' (' . $commune_nom . ')';
    } else {
        $ville = $ville_nom;
    }
    $commune = $commune_nom;
    ...
    if (empty($ville_nom)) {
        $errors[] = "Veuillez sélectionner votre Ville de résidence.";
    } elseif ($ville_raw === 'Autre' && empty($ville_autre)) {
        $errors[] = "Vous avez choisi « Autre ville ». Veuillez préciser le nom de votre ville.";
    }
    if (empty($commune_nom)) {
        $errors[] = "Veuillez sélectionner votre Commune ou Quartier de résidence.";
    } elseif (strpos($commune_raw, 'Autre') === 0 && empty($commune_autre)) {
        $errors[] = "Vous avez choisi une option « Autre » pour la commune/quartier. Veuillez préciser son nom.";
    }
    if (empty($activite)) {
    ...
    // Validation du mot de passe & confirmation (pour les nouveaux candidats non connectés)
    if (!$is_logged_in) {
        if (empty($password) || strlen($password) < 6) {
            $errors[] = "Veuillez définir un mot de passe d'au moins 6 caractères pour votre compte Promoteur.";
    ...
    if (!$is_logged_in && !empty($email)) {
        ...
        if ($st_check_u->fetch()) {
            $errors[] = "Cette adresse email ($email) est déjà associée à un compte TikeWA. Veuillez vous connecter pour associer ce compte à votre candidature, ou utiliser une autre adresse email pour un compte organisateur séparé.";
        }
    }
    ...
    if (!empty($email) || !empty($clean_tel)) {
        ...
        if ($existing_req) {
            if ($existing_req['statut'] === 'en_attente') {
                ...
                $errors[] = "Un compte promoteur approuvé existe déjà pour ces coordonnées. Vous pouvez vous connecter pour accéder directement à votre espace organisateur.";
            }
        }
    }
    ...
    if (isset($_FILES['piece_identite']) && $_FILES['piece_identite']['error'] === UPLOAD_ERR_OK) {
        ...
        $errors[] = "La pièce d'identité est obligatoire pour l'examen d'éligibilité promoteur.";
    }
    ...
    if (!empty($errors)) {
        ...
        try {
            ...
            if ($is_logged_in) {
                ...
                // Si l'utilisateur est déjà connecté, on conserve son statut de compte sans le verrouiller
            } else {
            ...
            $sql_req = "
                INSERT INTO promoter_requests (
                    ...
                    nom_complet, telephone, ville, commune, email, activite, experience, volume_estime,
...
<head>
    ...
    <title>Devenir Promoteur Organisateur - TikeWA</title>
    ...
    <style>
        ...
        /* Bannière Spécifique Organisateur Indépendant */
        .promoter-standalone-banner {
            background: #FFF8F5;
        ...
        .btn-submit-promo {
            ...
            gap: 0.5rem;
            transition: var(--eventia-transition);
            box-shadow: 0 4px 15px rgba(255, 74, 13, 0.3);
            margin-top: 1rem;
        }
        ...
        .btn-submit-promo:hover {
            ...
            box-shadow: 0 6px 20px rgba(255, 74, 13, 0.4);
        }
...
<body>
    ...
    <header class="promo-hero">
        <a href="client/accueil"
            style="color: #A3A3A3; text-decoration: none; font-weight: 500; font-size: 0.85rem; display: inline-flex; align-items: center; gap: 6px; margin-bottom: 1rem;">
            <i class="fa-solid fa-arrow-left"></i> Retour à la Billetterie
        </a>
        <span class="page-kicker" style="color: var(--accent, #FF4A0D); display: block; margin-bottom: 0.4rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; font-size: 0.82rem;">
            <i class="fa-solid fa-bullhorn"></i> Inscription & Éligibilité Organisateur
        </span>
        <h1>Devenez Promoteur Officiel sur TikeWA</h1>
        <p>
            Vendez vos billets en ligne, gérez vos jauges et plans de salle interactifs, et encaissez vos recettes
    ...
    <main class="promo-wrapper">
        <div class="promo-card">
                ...
                <div class="alert-box alert-<?php echo $msg_type; ?>">
                    ...
                    <?php if ($msg_type === 'success'): ?>
                        <div style="margin-top: 1.25rem; display: flex; flex-wrap: wrap; gap: 0.75rem; align-items: center;">
                            <a href="connexion"
                                style="background: #000000; color: #ffffff; padding: 0.65rem 1.25rem; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 0.88rem; display: inline-block;">
                                Aller à la page de connexion
                            </a>
                            <a href="<?php echo htmlspecialchars($wa_support_url); ?>" target="_blank" rel="noopener noreferrer"
                                style="background: #25D366; color: #ffffff; padding: 0.65rem 1.25rem; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 0.88rem; display: inline-flex; align-items: center; gap: 6px;">
            ...
            <?php if ($msg_type !== 'success'): ?>
                ...
                <form method="POST" action="devenir-promoteur" enctype="multipart/form-data" id="promoterForm" novalidate>
                    ...
                    <!-- 3. COORDONNÉES & RESPONSABLE DU COMPTE -->
                    <div class="section-title">
                    ...
                    <div class="form-row-2">
                        <div class="form-group">
                            <label for="email">Adresse Email professionnelle de l'organisateur</label>
                            ...
                            <small style="color: var(--muted); font-size: 0.78rem;">Identifiant officiel pour votre espace promoteur.</small>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <small style="color: var(--muted); font-size: 0.78rem;">Utilisé pour vos reversements de recettes et alertes.</small>
                        </div>
                    </div>

                    <!-- Localisation : Ville et Commune en listes déroulantes synchronisées -->
                    <div class="form-row-2">
                        <div class="form-group">
                            <label for="ville"><i class="fa-solid fa-city" style="color: var(--eventia-amber, #FF4A0D);"></i> Ville de résidence *</label>
                            <select id="ville" name="ville" required onchange="handleVilleChange(this.value)">
                                <option value="">-- Sélectionnez votre ville --</option>
                                <?php
                                $villes_ci = [
                                    'Abidjan' => 'Abidjan (District Autonome)',
                                    'Yamoussoukro' => 'Yamoussoukro (Capitale)',
                                    'Bouaké' => 'Bouaké (Gbêkê)',
                                    'San-Pédro' => 'San-Pédro (Bas-Sassandra)',
                                    'Grand-Bassam' => 'Grand-Bassam (Sud-Comoé)',
                                    'Assinie' => 'Assinie-Mafia',
                                ...
                                $cur_ville = $_POST['ville'] ?? 'Abidjan';
                                foreach ($villes_ci as $vk => $vl) {
                                    $sel = ($cur_ville === $vk) ? 'selected' : '';
                                    echo "<option value=\"$vk\" $sel>$vl</option>";
                                }
                                ?>
                            </select>
                            <div id="box_ville_autre" style="display: <?php echo ($cur_ville === 'Autre') ? 'block' : 'none'; ?>; margin-top: 6px;">
                                <input type="text" id="ville_autre" name="ville_autre" placeholder="Précisez le nom de votre ville..." value="<?php echo htmlspecialchars($_POST['ville_autre'] ?? ''); ?>">
                            </div>
                        </div>

                        <div class="form-group">
                            <label for="commune"><i class="fa-solid fa-location-dot" style="color: var(--eventia-amber, #FF4A0D);"></i> Commune / Secteur *</label>
                            <select id="commune" name="commune" required onchange="handleCommuneChange(this.value)">
                                <option value="">-- Sélectionnez d'abord une ville --</option>
                            </select>
                            <div id="box_commune_autre" style="display: none; margin-top: 6px;">
                                <input type="text" id="commune_autre" name="commune_autre" placeholder="Précisez votre commune ou quartier..." value="<?php echo htmlspecialchars($_POST['commune_autre'] ?? ''); ?>">
                            </div>
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="piece_identite"><span id="lbl_piece_id">Pièce d'identité de l'organisateur (CNI / Passeport)</span> *</label>
                        <div class="upload-field-box">
                    ...
                    <?php if (!$is_logged_in): ?>
                        <div class="form-row-2">
                            <div class="form-group">
                                <label for="password"><i class="fa-solid fa-lock"></i> Définir votre Mot de passe Espace Promoteur</label>
                    ...
                    <div class="form-group">
                        <label for="description">Description de vos activités événementielles</label>
                        <textarea id="description" name="description" rows="4" required
                            placeholder="Décrivez vos activités principales, vos types d'événements, votre public cible et vos projets..."><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
                        <small style="color: var(--muted); font-size: 0.78rem;">Présentez la nature des événements que vous commercialiserez sur TikeWA.</small>
                    </div>
                    ...
                    <div
                        ...
                        En soumettant ce dossier, vous certifiez l'exactitude des informations fournies. L'activation définitive de votre Espace Promoteur est opérée sous 24h par l'administration de TikeWA après vérification de vos pièces justificatives.
                    </div>

                    <button type="submit" class="btn-submit-promo" id="btnSubmitPromo">
                        <i class="fa-solid fa-paper-plane" id="btnSubmitIcon"></i>
                        <span id="btnSubmitText">Soumettre ma Demande d'Éligibilité Promoteur</span>
                    </button>
    ...
    <script>
        // Dictionnaire des communes et quartiers par ville de Côte d'Ivoire
        const COMMUNES_BY_VILLE = {
            'Abidjan': [
                'Cocody',
                'Yopougon',
                'Plateau',
        ...
        // Gestionnaire de synchronisation Ville -> Commune
        function handleVilleChange(selectedVille, preselectedCommune = null) {
            const selectCommune = document.getElementById('commune');
            const boxVilleAutre = document.getElementById('box_ville_autre');
            const inputVilleAutre = document.getElementById('ville_autre');
            const boxCommuneAutre = document.getElementById('box_commune_autre');
            const inputCommuneAutre = document.getElementById('commune_autre');

            if (!selectCommune) return;

            // Affichage du champ libre "Autre ville"
            if (selectedVille === 'Autre') {
                if (boxVilleAutre) boxVilleAutre.style.display = 'block';
                if (inputVilleAutre) {
                    inputVilleAutre.required = true;
                    inputVilleAutre.focus();
                }
            } else {
                if (boxVilleAutre) boxVilleAutre.style.display = 'none';
                if (inputVilleAutre) inputVilleAutre.required = false;
            }
            ...
            if (!selectedVille) {
                ...
                opt.value = '';
                opt.textContent = '-- Sélectionnez d\'abord une ville --';
                selectCommune.appendChild(opt);
                if (boxCommuneAutre) boxCommuneAutre.style.display = 'none';
                if (inputCommuneAutre) inputCommuneAutre.required = false;
                return;
            }

            const optDefault = document.createElement('option');
            optDefault.value = '';
            optDefault.textContent = '-- Choisissez votre commune / quartier --';
            selectCommune.appendChild(optDefault);

            const communes = COMMUNES_BY_VILLE[selectedVille] || ['Centre-Ville / Quartier principal', 'Autre quartier...'];
            
            communes.forEach(c => {
                const opt = document.createElement('option');
                opt.value = c;
                opt.textContent = c;
                if (preselectedCommune && (preselectedCommune === c || preselectedCommune.toLowerCase() === c.toLowerCase())) {
                    opt.selected = true;
                }
                selectCommune.appendChild(opt);
            });

            // Cas d'une ville personnalisée 'Autre'
            if (selectedVille === 'Autre') {
                selectCommune.innerHTML = '';
                const optAutreDef = document.createElement('option');
                optAutreDef.value = '';
                optAutreDef.textContent = '-- Précisez votre commune / secteur --';
                selectCommune.appendChild(optAutreDef);

                const opt1 = document.createElement('option');
                opt1.value = 'Centre-Ville';
                opt1.textContent = 'Centre-Ville / Zone Principale';
                selectCommune.appendChild(opt1);

                const opt2 = document.createElement('option');
                opt2.value = 'Autre commune...';
                opt2.textContent = '✏️ Autre commune ou quartier (préciser)...';
                selectCommune.appendChild(opt2);
            }

            // Si une commune personnalisée a été soumise précédemment et n'est pas dans la liste standard
            if (preselectedCommune && !communes.includes(preselectedCommune)) {
                let foundMatch = false;
                for (let i = 0; i < selectCommune.options.length; i++) {
                    if (selectCommune.options[i].value === preselectedCommune) {
                ...
                if (!foundMatch) {
                    for (let i = 0; i < selectCommune.options.length; i++) {
                        if (selectCommune.options[i].value.startsWith('Autre')) {
                            selectCommune.selectedIndex = i;
                            if (inputCommuneAutre) inputCommuneAutre.value = preselectedCommune;
                            break;
                        }
                    }
                }
            ...
            handleCommuneChange(selectCommune.value);
        }

        function handleCommuneChange(selectedCommune) {
            const boxCommuneAutre = document.getElementById('box_commune_autre');
            const inputCommuneAutre = document.getElementById('commune_autre');
            if (!boxCommuneAutre || !inputCommuneAutre) return;

            if (selectedCommune && (selectedCommune.startsWith('Autre') || selectedCommune === 'Autre')) {
                boxCommuneAutre.style.display = 'block';
                inputCommuneAutre.required = true;
                inputCommuneAutre.focus();
            } else {
                boxCommuneAutre.style.display = 'none';
                inputCommuneAutre.required = false;
            }
        }
        ...
        if (formPromo) {
            formPromo.addEventListener('submit', function(e) {
                // Contrôle côté client du mot de passe si présent
                if (pwdInput && pwdConfInput) {
                ...
                // Contrôle côté client pour "Autre ville" et "Autre commune"
                const selectVille = document.getElementById('ville');
                const inputVilleAutre = document.getElementById('ville_autre');
                if (selectVille && selectVille.value === 'Autre' && inputVilleAutre && !inputVilleAutre.value.trim()) {
                    e.preventDefault();
                    alert("Vous avez sélectionné « Autre ville ». Veuillez préciser le nom de votre ville.");
                    inputVilleAutre.focus();
                    return;
                }
                ...
                const selectCommune = document.getElementById('commune');
                const inputCommuneAutre = document.getElementById('commune_autre');
                if (selectCommune && selectCommune.value && selectCommune.value.startsWith('Autre') && inputCommuneAutre && !inputCommuneAutre.value.trim()) {
                    e.preventDefault();
                    alert("Vous avez sélectionné une option « Autre » pour la commune. Veuillez préciser son nom.");
                    inputCommuneAutre.focus();
                    return;
                }
        ...
        // Auto-scroll vers le message d'alerte s'il existe et initialisation Ville/Commune
        window.addEventListener('DOMContentLoaded', function() {
            ...
            // Initialisation de la commune selon la ville sélectionnée
            const selectVille = document.getElementById('ville');
            if (selectVille && selectVille.value) {
                const initCommune = <?php echo json_encode($_POST['commune'] ?? 'Cocody'); ?>;
                handleVilleChange(selectVille.value, initCommune);
            }
        });
    </script>
