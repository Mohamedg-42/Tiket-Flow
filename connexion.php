...
// 3. Traitement du formulaire de connexion avec limitation des tentatives (SEC-007)
$max_attempts = 5;
$lockout_seconds = 900; // 15 minutes

if (isset($_SESSION['login_lockout_until']) && time() < $_SESSION['login_lockout_until']) {
    $mins = ceil(($_SESSION['login_lockout_until'] - time()) / 60);
    $error = "Trop de tentatives infructueuses. Votre accès est temporairement verrouillé. Veuillez patienter $mins minute(s).";
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            if (!$statusCheck['allowed']) {
                ...
                // Régénération de l'identifiant de session anti-fixation (SEC-006)
            ...
            if ($attempts_left <= 0) {
                ...
                $error = "Trop de tentatives infructueuses. Votre accès est temporairement verrouillé pendant 15 minutes.";
                logActivity('connexion.lockout', 'user', null, "Verrouillage temporaire après $max_attempts échecs pour : " . substr($email, 0, 80));
            } else {
                $error = "Adresse email ou mot de passe incorrect. ($attempts_left tentative(s) restante(s))";
...
<body>
    ...
    <div class="auth-container eventia-card">
        ...
        <!-- Message d'erreur -->
        <?php if (!empty($error)): ?>
            <div class="eventia-alert eventia-alert-error"
                style="background: #F5F5F5; border: 1px solid #E5E5E5; color: #000000; border-radius: 10px; padding: 0.75rem 1rem; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 8px; margin-bottom: 1.25rem;">
        ...
        <?php if (!empty($success_msg)): ?>
            <div class="eventia-alert eventia-alert-success"
                style="background: #FFF2ED; border: 1px solid #FFF2ED; color: #FF4A0D; border-radius: 10px; padding: 0.75rem 1rem; font-size: 0.85rem; font-weight: 600; display: flex; align-items: center; gap: 8px; margin-bottom: 1.25rem;">
        ...
        <form method="POST" action="connexion.php" style="display: flex; flex-direction: column; gap: 1.15rem;">
            ...
            <div class="eventia-form-group" style="display: flex; flex-direction: column; gap: 6px;">
                ...
                <input type="email" id="email" name="email" required placeholder="nom@exemple.com"
                    value="<?php echo htmlspecialchars($_POST['email'] ?? ''); ?>" autocomplete="email" autofocus
